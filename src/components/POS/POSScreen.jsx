...
export default function POSScreen({ issuers, productsDB, salesDB = [], recordSale, customersDB, recordCustomer }) {
  ...
  const getNormalizedCartItems = (cartItems) => {
    const legalNames = {
    ...
    return cartItems.map(item => {
      const rawOwner = item.ownerName || item.ownerId || item.propietario || item.owner;
      let ownerId = 'edgar';
      let ownerShortName = 'Edgar';
      let ownerLegalName = legalNames.edgar;
      ...
      if (rawOwner) {
        ...
        if (s.includes('fabian') || s.includes('fabián') || s.includes('domingo') || s.includes('junior') || s.includes('carlos')) {
          ...
          ownerId = 'amparito';
          ownerShortName = 'Amparito';
          ownerLegalName = legalNames.amparito;
        }
      } else {
  ...
  const [transferRecipientId, setTransferRecipientId] = useState('edgar'); // brother user id
  ...
  const handleActivateMixedPayment = () => {
    ...
    setPaymentsList([
      ...
      },
      {
        ...
        amount: half2,
        recipientName: transferRecipient || 'Edgar',
        recipientId: transferRecipientId || 'Edgar',
        bank: transferBank || '',
  ...
  const handleUpdatePaymentLine = (id, field, value) => {
    setPaymentsList(prev => prev.map(p => {
      ...
      if (field === 'recipientName') {
        ...
        updated.recipientName = normalizedName;
        updated.recipientId = normalizedId;
      }
      return updated;
    }));
  };
  ...
  const syncSecondaryCustomerScreen = (updatedCust) => {
  ...
  const handleCustomerChange = (e) => {
    ...
    setCustomer(prev => {
      ...
      syncSecondaryCustomerScreen(updated);
      return updated;
    });
  };
  ...
  const emitirNotaVentaInterna = async (withPrint, issuerData, paymentDetails, totalsData, transactionId) => {
    ...
    try {
      ...
      const resultNV = await runTransaction(db, async (t) => {
        ...
        const normalizedCart = getNormalizedCartItems(cart);
        ...
        if (paymentMethod === 'TRANSFERENCIA') {
          ...
          recipientName = transferRecipient || 'Edgar';
          recipientType = 'OWNER';
        } else if (paymentMethod === 'MIXTO' && paymentDetails?.payments) {
          const trPayment = paymentDetails.payments.find(p => p.method === 'TRANSFERENCIA');
          if (trPayment) {
            recipientId = trPayment.recipientId || 'edgar';
            recipientName = trPayment.recipientName || 'Edgar';
            recipientType = 'OWNER';
          }
        }
        ...
        const nuevaVentaInterna = {
          ...
          transferRecipient: recipientName,
        ...
        <div className="cart-container">
          ...
          <div className="cart-summary" style={{ flex: '1 1 auto', overflowY: 'auto' }}>
            ...
            <div className="summary-row">
              <span>Subtotal</span>
              <span>${(isNotaVenta ? total : subtotal).toFixed(2)}</span>
            </div>
            {!isNotaVenta && (
              <>
                <div className="summary-row">
        ...
        <div className="modal-overlay animate-fade-in" style={{ zIndex: 1000, background: 'rgba(15, 23, 42, 0.85)', backdropFilter: 'blur(4px)' }}>
          <div className="glass-panel" style={{ padding: '2.5rem', textAlign: 'center', maxWidth: '380px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', border: '1px solid var(--panel-border)', borderRadius: '12px' }}>
            ...
            <h3 style={{ margin: 0, color: 'var(--text-main)', fontSize: '1.25rem' }}>
              {isNotaVenta ? 'Registrando Venta Interna' : 'Procesando Factura SRI'}
            </h3>
